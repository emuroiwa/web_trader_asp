    ...
    Protected Sub cmbUnsecured_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbUnsecured.SelectedIndexChanged
        ...
        If cmbUnsecured.SelectedValue.ToString <> "" Then
            Try
                ...
                'validate username first
                strSQL = "select  deals_live.dealamount,securitypurchase.interestrate,securitypurchase.amtavailable,securitypurchase.maturitydate from securitypurchase join deals_live on securitypurchase.tb_id=deals_live.TB_ID  " & _
                ...
                drSQL = cmSQL.ExecuteReader()
                Do While drSQL.Read
                    PurValue.Text = drSQL.Item(0).ToString
                    IntRate.Text = drSQL.Item(1).ToString
                    AvalableForSale.Text = CDec(drSQL.Item(2).ToString) - SecurityAttached(Trim(cmbUnsecured.SelectedValue.ToString()), Trim(getRef(cmbUnsecured.SelectedValue.ToString)))
                    
                    'maturitySecurity.Text = Trim(drSQL.Item(3).ToString)
                    'DaysMaturity.Text = DateDiff(DateInterval.Day, CDate(Session("SysDate")), CDate(drSQL.Item(4)))
