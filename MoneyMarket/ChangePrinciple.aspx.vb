...
Public Class ChangePrinciple
            ...
            If Not Request.ServerVariables("QUERY_STRING") Is Nothing Then
                ...
                Try
                    ...
                    If Trim(DealType) = "Discount sale" Then
                        ...
                        lblError.Text = alert("This deal cannot be ammended.", "Deal Principle")
                        btnValidate.Enabled = False
                        btnSave.Enabled = False
                        btnReset.Enabled = False
                        Exit Sub
                ...
                Catch ex As NullReferenceException
                    lblError.Text = alert("Select a deal that you want to change its Principle.", "Deal Principle")
                    btnValidate.Enabled = False
                    btnSave.Enabled = False
                    btnReset.Enabled = False
                ...
                End Try
            Else
                lblError.Text = alert("Select a deal that you want to change Principle.", "Incomplete informaton")
                btnValidate.Enabled = False
                btnSave.Enabled = False
                btnReset.Enabled = False
            End If
    ...
    Protected Sub btnValidate_Click(sender As Object, e As EventArgs) Handles btnValidate.Click
        ...
        '*********************************************************************
        If RdAMendType.SelectedValue = "Increase" Then
            AmendType = "Principal Increase"
        Else
            AmendType = "Principle Decrease"
        End If
        If Trim(roll) = "P" Then
        ...
        Select Case AmendType
            ...
            Case "Principal Increase"
                ...
                If txtReason.Text = "" Then
                    MsgBox("Enter the reason for principal change.", MsgBoxStyle.Exclamation, "Reason")
                    Exit Sub
                End If

                If CDec(txtPrincipalChange.Text) < CDec(LblAmount.Text) Then
                    MsgBox("Transaction is to increase principal, but amount entered is less than the current deal amount", MsgBoxStyle.Exclamation, "Increase Principal")
                    Exit Sub
                End If

                If CDec(txtPrincipalChange.Text) = CDec(LblAmount.Text) Then
                    MsgBox("Transaction is to increase principal, but amount entered is less equal to the current deal amount", MsgBoxStyle.Exclamation, "Increase Principal")
                    Exit Sub
                End If
                ...
                Call limitsch.getDealTypeStructure(dealcode)
                ...
                TranxLimitVal = limitsch.PeriodLimitValidation(Int(lblTenor.Text), getDealType(GetDealCode(Trim(lblRef.Text))), Trim(Session("username")), _
                                CDbl(txtPrincipalChange.Text), Trim(lblCurrency.Text), Trim(txtDealtype.Text))
                ...
                If TranxLimitVal(0) = "1" Then
                    MsgBox("Transaction Limit exceeded" & vbCrLf & _
                          ...
                          "****************************************************" & vbCrLf & _
                           ...
                           "Transaction Amount :   " & txtPrincipalChange.Text, MsgBoxStyle.Information, "Limits")


                    If getSysParam("tranlmt") = "warn" Then
                        MsgBox("Transaction Limit violation will be recorded", MsgBoxStyle.Exclamation, "Transaction Limit exceeded")
                ...
                If limitsch.CheckCounterpartyLimit(Trim(lblCustomerNumber.Text), Session("DealStructure"), CDec(txtPrincipalChange.Text), Trim(lblCurrency.Text), CDate(dtMaturity.Text)) = False Then

                    crStatus = "1"
                    If CounterpartyLimitViolation(CDec(txtPrincipalChange.Text), Session("DealStructure")) = "stop" Then
                        Exit Sub
                    End If
                Else
                    crStatus = "2"
            ...
            Case "Principle Decrease"
                ...
                If CDec(txtPrincipalChange.Text) > CDec(LblAmount.Text) Then
                    MsgBox("Transaction is to increase principal, but amount entered is greater than the current deal amount", MsgBoxStyle.Exclamation, "Increase Principal")
                    Exit Sub
                End If
    ...
    Protected Sub RdAMendType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles RdAMendType.SelectedIndexChanged
        If RdAMendType.SelectedValue = "Increase" Then
            AmendType = "Principle Increase"
        Else
            AmendType = "Principle Decrease"
        End If
    End Sub
