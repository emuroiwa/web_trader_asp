...
Public Class MMDeposit
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            tabMMDeposit.ActiveTabIndex = 0
            GetCustomerCollateral(txtCustomerNameDeposit.Text)
            'URL variables
            lblDays.Text = InterestBasisValue

            Call Deal_Instructions()
            Call DealDetails(Request.QueryString("dealreference"))
            Call load_TBs()
            Call GetIntBasisValue()
        End If
    End Sub
    Private Sub DealDetails(ByVal ref As String)
        ...
        Try
            ...
            strSQLx = "select DEALS_MATURED.PortfolioID as ID,* from deals_matured join customer on deals_matured.customernumber=customer.customer_number join portfoliostructure " & _
                          " on deals_matured.portfolioid=portfoliostructure.portfolioid join dealtypes on deals_matured.dealtype=dealtypes.deal_code " & _
                          " where  dealreference='" & ref & "'"
            ...
            Dim DealDescription As String
            Dim DealInstruction As String

            Do While drSQLx.Read

                If Trim(drSQLx.Item("dealcancelled").ToString) = "R" Then
                    lblError.Text = alert("This deal has been flagged as rolled over already.", "Load Deals")
                    ' Close and Clean up objects
                    Response.Redirect("MaturitiesMaintanance.aspx")
                    Exit Sub
                End If

                Dim ID As String
                ID = Trim(drSQLx.Item("ID").ToString)
                ' txtDashRef.Text = Trim(drSQL.Item("dealreference").ToString)
                txtCustomerNameDeposit.Text = Trim(drSQLx.Item("fullname").ToString)
                txtDealAmountDeposit.Text = drSQLx.Item("maturityamount").ToString
                txtIntRateDeposit.Text = Trim(drSQLx.Item("interestrate").ToString)
                txtTenorDeposit.Text = Trim(drSQLx.Item("tenor").ToString)
                StartDateDeposit.Text = CDate(Session("SysDate"))
                MaturityDateDeposit.Text = CDate(Session("SysDate"))
                customerA = Trim(drSQLx.Item("customernumber").ToString)
                lblCurrency.Text = drSQLx.Item("currency").ToString
                DealPortfolio = (Trim(drSQLx.Item("portfolioname").ToString))
                DealDescription = (Trim(drSQLx.Item("dealtypeDescription").ToString))
                DealInstruction = Trim(drSQLx.Item("instruction").ToString)
                lblDays.Text = drSQLx.Item("intdaysbasis").ToString
                ''exsiting deposit variables


                lblCurrency.Text = Trim(drSQLx.Item("currency").ToString)
                lblPortfolioDeposit.Text = DealPortfolio
                lblDealtypeDeposit.Text = Trim(drSQLx.Item("dealtype").ToString)
                lblProduct.Text = DealDescription
                If Int(Trim(drSQLx.Item("taxrate").ToString)) > 0 Then
                    chkTaxStatus.Checked = True
                Else
                    chkTaxStatus.Checked = False
                End If
                Call GetHist(customerA, "Basic Deposit", Trim(drSQLx.Item("currency").ToString))
                Call GetCustomerAddressInfo(customerA)
            Loop
        ...
        Catch er As Exception
            lblError.Text = alert(er.Message, "Get Status")
        End Try
    ...
    Protected Sub btnValidateDeposit_Click(sender As Object, e As EventArgs) Handles btnValidateDeposit.Click
        ...
        'ernest security
        'check Security
        'If cmbSecureDeposit.Text = "Yes" Then
        ...
        '        MsgBox("Insuficient Security for this deal", MsgBoxStyle.Information, "Security")
        '    End If
        '    'Check expiry dates of the collateral assigned
        '    Dim ttr As ListViewItem
        '    For Each ttr In lstSecurityList.Items
        '        If CDate(ttr.SubItems(3).Text) < CDate(MaturityDateDeposit.Value) Then
        '            ttr.BackColor = Color.Red
        '            ttr.ForeColor = Color.White
        '            MsgBox("Collateral / Security item ref: " & Trim(ttr.Text) & " Expires before deal matures", MsgBoxStyle.Information)
        '        End If
        '    Next
        'End If
        ...
        If chkRollover.Checked = True Then
            ...
            If Trim(cmbIntOpt.Text) = "0" Then
                ...
                tabMMDeposit.ActiveTabIndex = 2
                Exit Sub
            End If
            'ernest roll over
            'If RadioDays.Checked = False And RadioDate.Checked = False Then
            '    MsgBox("No frequency option selected for rollover.", MsgBoxStyle.Information)
            '    TabControl2.SelectedTab = TabControl2.TabPages(2)
            '    Exit Sub
            'End If

            'Check frequency. Frequency set to days
            'If RadioDays.Checked = True Then
            '    If rolloverDays.Value = 0 Then
            '        MsgBox("Frequency days value must not be 0.", MsgBoxStyle.Information)
            '        TabControl2.SelectedTab = TabControl2.TabPages(2)
            '        Exit Sub
            '    End If
            ...
            End If
            'ernest quesition
            'If (mmdeal_object.NonBusinessDay(DateAdd(DateInterval.Day, rolloverDays.Text, StartDateDeposit.Text)) = True) _
            '   Or (mmdeal_object.Holidays(DateAdd(DateInterval.Day, rolloverDays.Text, StartDateDeposit.Text), Trim(Request.QueryString("currency")) = True)) Then
        ...
        If UCase(Trim(Request.QueryString("dash"))) = "FALSE" Then 'Check limits only if the deal is not on the dash board
            ...
            If TranxLimitVal(0) = "1" Then
                ...
                If getSysParam("tranlmt") = "warn" Then
                    lblWarning.Text = alert("Limit violation will be recorded", "Transaction Limit exceeded")
            ...
            CustName = txtCustomerNameDeposit.Text
    ...
    Protected Sub cmdSaveDealDeposit_Click(sender As Object, e As EventArgs) Handles cmdSaveDealDeposit.Click
        ...
        customerA = txtCustomerNameDeposit.Text
    ...
    Private Function GetFieldVal(ByVal fldkey As String, ByVal returnVal As String, dealtype As String) As Decimal
        ...
        Catch eb As Exception
            lblWarning.Text = warning(eb.Message, "Error")
        End Try
    ...
    Private Function CounterpartyLimitViolation(dealamt As Double, dealtype As String, curr As String, matDate As String) As String
        ...
        Else
            ...
            lblWarning.Text = alert("Counterparty Limit exceeded" & vbCrLf & _
                      ...
                      "Transaction Amount :   " & Format(dealamt, "###,###.00"), "Limits")
        End If

        If getSysParam("tranlmt") = "warn" Then
            lblWarning.Text = warning("Counterparty Limit violation will be recorded", "Counterparty Limit exceeded")
            x = "warn"
        ElseIf getSysParam("tranlmt") = "stop" Then
   ...
   ByVal ethixinterestaccount As String, ByVal ethixcommissionAccount As String, ByVal debitAccrual As String, ByVal creditAccrual As String)
    ...
    Private Function GetSettlementDetailsPar() As Boolean
        ...
        Catch ex As SqlException
            ...
            object_userlog.SendDataToLog(Session("loggedUserLog") & "ERR001" & Format(Now, "dd/MM/yyyy hh:mm:ss") & "GetIntBasisValue" & ex.Message, Session("serverName"), Session("client"))
            '************************END****************************************
        End Try
    End Function
    ...
    Protected Sub chkPledge_CheckedChanged(sender As Object, e As EventArgs) Handles chkPledge.CheckedChanged
        ...
        RefreshContracts(txtCustomerNameDeposit.Text, True)
        tabMMDeposit.ActiveTabIndex = 3
        'MsgBox("gggg")
    ...
    Protected Sub btnSaveNew_Click(sender As Object, e As EventArgs) Handles btnSaveNew.Click
        ...
        MsgBox("sdjk")
        If Trim(txtCustomerNameDeposit.Text) = " " Then
        ...
        Try
            strSQL5 = "insert loan_pledges values('" & Trim(txtContractRefNew.Text) & "','" & Trim(txtCustomerNameDeposit.Text) & "'," & txtAmt.Text _
            ...
            'Log the event *****************************************************
            object_userlog.SendDataToLog(Session("loggedUserLog") & "PLG001" & Format(Now, "dd/MM/yyyy hh:mm:ss") & " Contract Reference" & Trim(txtContractRefNew.Text) & " Customer :" & Trim(txtCustomerNameDeposit.Text) & " Contract Value " & txtAmt.Text _
            ...
            Call RefreshContracts(txtCustomerNameDeposit.Text, True)
    ...
    Protected Sub cmdSale_Click(sender As Object, e As EventArgs) Handles cmdSale.Click
        ...
        InsertSecurity(Trim(cmbTB.SelectedValue.ToString), Trim(Get_TB(cmbTB.SelectedValue.ToString)), txtSale.Text, txtExpiry.Text, Trim(Session("username")), Trim(txtCustomerNameDeposit.Text))
        ...
        Session("ernest") = "True"
        Try
            strSQL = " SELECT * FROM SECURITYTEMP  where Customer='" & Trim(txtCustomerNameDeposit.Text) & "' and Dealer='" & Trim(Session("username")) & "'"
    ...
    Protected Sub btnAddCollateralLoan_Click(sender As Object, e As EventArgs) Handles btnAddCollateralLoan.Click
        ...
        InsertSecurity(Trim(cmbSecurity.SelectedValue.ToString), Trim(CollateralDescription(cmbSecurity.SelectedValue.ToString)), txtCollateralLoan.Text, txtExpiry.Text, Trim(Session("username")), Trim(txtCustomerNameDeposit.Text))
        Dim message As String = "Security Has Been Added"""
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "myFunction", "MsgBox('" & message & "');", True)
        Session("ernest") = "True"
        Try
    ...
    Private Sub AttachSecurity(ByVal dealNum As String)
        ...
        Try
            strSQLdd = "SELECT Security,DealRef,Amount,id FROM SECURITYTEMP  where Customer='" & Trim(txtCustomerNameDeposit.Text) & "' and Dealer='" & Trim(Session("username")) & "'"
            cnSQLdd = New SqlConnection(Session("ConnectionString"))
            cnSQLdd.Open()
            cmSQLdd = New SqlCommand(strSQLdd, cnSQLdd)
            drSQLdd = cmSQLdd.ExecuteReader
    ...
    Private Function GetTotalSecurity() As Decimal
        ...
        Try
            ...
            strSQL2 = "select sum(amount) FROM SECURITYTEMP  where Customer='" & Trim(txtCustomerNameDeposit.Text) & "' and Dealer='" & Trim(Session("username")) & "'"
